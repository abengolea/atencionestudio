    ...
    <div className="flex flex-col gap-8">
      <div>
          <Button variant="outline" size="sm" asChild>
              <Link href="/cases"><ArrowLeft size={16} className="mr-2"/> Back to Cases</Link>
          </Button>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        ...
        <div className="space-y-8">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="font-headline">Case Summary</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">Case Type</span><Badge variant="secondary">{caseDetails.type}</Badge></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Urgency</span><Badge variant={caseDetails.urgency === 'high' ? "destructive" : "outline"}>{caseDetails.urgency}</Badge></div>
                <Separator />
                <div className="flex items-center gap-3"><DollarSign size={16} className="text-muted-foreground" /><span className="font-medium">Estimated Value: {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(caseDetails.estimatedValue)}</span></div>
                <div className="flex items-start gap-3"><Calendar size={16} className="text-muted-foreground mt-1" /><p>{caseDetails.timeline}</p></div>
                <Separator />
                <div>
                    <h4 className="font-semibold mb-2 flex items-center gap-2"><FileText size={16} />Documents</h4>
           ...
           <Card>
            <CardHeader>
              ...
              <CardDescription>Record your decision and notes for this case.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                <Textarea placeholder="Add private notes..."/>
                <div className="flex gap-2">
                    <Button className="w-full bg-green-600 hover:bg-green-700"><Check className="mr-2" size={16} /> Accept Case</Button>
                    <Button variant="destructive" className="w-full"><X className="mr-2" size={16} /> Reject Case</Button>
                </div>
            </CardContent>
           </Card>
